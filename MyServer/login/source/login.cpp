...
login::login(SSL_CTX* tmp_ssl_ctx_fd,
             std::condition_variable* tmp_local_msg_queue_cv = nullptr,
             std::mutex* tmp_local_msg_queue_mtx = nullptr,
             std::queue<local_msg_type_t>* tmp_local_msg_queue = nullptr) {
...
login::~login() {
    for (auto i : socket_catalogue) {
        SSL_shutdown(sockets[i].ssl_fd);
...
bool login::db_init() {
    int status = sqlite3_prepare_v2(db_sqlite, json_config["login"]["sql_to_exec_get_passwd"].asCString(), strlen(json_config["login"]["sql_to_exec_get_passwd"].asCString()), &db_sqlite_stmt_getPassword, nullptr);
...
std::string login::db_get_userlist() {
    ...
    //读取完了之后，status 就是 SQLITE_DONE
    while (status == SQLITE_ROW) {
        //以JSON格式、字符串读取所有查询结果，这样就避免了数据库列类型不一致的问题，反正都出来都是 string
...
bool login::db_verify(const char* name, const char* passwd) {
    ...
    //绑定sql语句中的变量为要查询的名字，这样就能获得对应的密码
