...
Message_router::Message_router(SSL_CTX* tmp_ssl_ctx_fd = nullptr,
                               std::condition_variable* tmp_local_queue_cv = nullptr,
                               std::mutex* queue_mtx = nullptr,
                               std::queue<struct local_msg_type_t>* queue_ptr = nullptr) {
    ...
    log_file.open(json_config["message_router"]["log_file_path"].asString(), std::ios::out | std::ios::app);
    if (!log_file.is_open()) {
        this->success_tag = -2;
        return;
    }
    ...
    epoll_events_ptr = new (epoll_event[json_config["message_router"]["max_socket_num"].asInt()]);
    ...
    while (1) {
        ...
        while (local_msg_queue->size() > 0) {
            ...
            if (type == LOCAL_MSG_TYPE_USER_LIST) {
                ...
                //用户登录成功,把所有的用户列表发送给该用户，方便他的界面显示
                ...
                if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, tmp_local_message.socket_fd, &tmp_event) < 0) {
                    ...
                    if (tmp_msg_map_iterator != to_be_sent_message_map.end()) {
                        for (auto tmp_iterator = tmp_msg_map_iterator->second.begin();
                             tmp_iterator != tmp_msg_map_iterator->second.end();
                             ++tmp_iterator) {
...
void Message_router::message_worker(void) {
    ...
    while (1) {
        ...
        bzero(epoll_events_ptr,  epoll_events_num * sizeof(epoll_event));
        bzero(data_buffer, max_buffer_size);
    ...
    while (1) {
        ...
        for (int i = 0; i < tmp_message_to_handle; ++i) {
            ...
            if (user_map.count(tmp_message.receiver) > 0 && user_map[tmp_message.receiver].is_down != true) {
                tmp_status = SSL_write(user_map[tmp_message.receiver].ssl_fd, tmp_message.content.c_str(), tmp_message.content.size());
                //每条消息的分隔符
...
void Message_router::cleaner(void) {
    ...
    while (1) {
        ...
        for (auto i : tmp_to_be_closed_vector) {
            tmp_user_item = user_map[i];
            user_rindex.erase(tmp_user_item.socket_fd);
            user_map.erase(i);
            user_index.erase(user_index.begin());
